...
class MinCostTargetRateAgent(BaseAgent):
    ...
    def predict(self, obs, deterministic: bool = True):
        ...
        model_indices = select_models(self.env, slice_idxs, "min_vcpu", self.rng)
        prbs = select_prbs(
            self.env, users, slice_idxs, model_indices, "target_rate", self.rng
        )
    ...
    def save(self, path: str) -> None:
        raise NotImplementedError("MinCostTargetRateAgent does not support saving.")

    @classmethod
    def load(cls, path: str):
        raise NotImplementedError("MinCostTargetRateAgent does not support loading.")
